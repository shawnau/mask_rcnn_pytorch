...
def make_one_rcnn_target(cfg, proposals, truth_boxes, truth_labels):
    ...
    sampled_target   = torch.from_numpy(sampled_target  ).to(cfg.device)
    sampled_label    = torch.from_numpy(sampled_label   ).to(cfg.device)
    sampled_proposal = torch.from_numpy(sampled_proposal).to(cfg.device)
    return sampled_proposal, sampled_label, sampled_assign, sampled_target
...
def make_rcnn_target(cfg, proposals, truth_boxes, truth_labels):
    ...
    print('sampled_label before stack: ', sampled_labels)
    ...
    print('sampled_labels after stack: ', sampled_labels)
